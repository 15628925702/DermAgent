...
**短标题：** DermAgent 皮肤科多模态推理框架  
**关键词：** 皮肤科人工智能；医疗 Agent；视觉语言模型；检索增强诊断；多模态推理；公平性评估

## 亮点
- 提出一个皮肤科 Agent 框架，在保持底层视觉语言模型不变的前提下，将感知、检索、专科推理、聚合和报告显式拆开。
- 在 PAD-UFES-20 上建立了同模型、同样本、冻结评估的公平比较协议，并在该协议下显著优于直接 Qwen 推理。
- 消融实验表明检索是当前系统最大的收益来源，经过校准后的元数据与 learned scorer 在进一步训练后也开始稳定发挥作用。
- 完成了 DDI 外部评估流程，并明确将其定位为外部二分类鲁棒性 / 公平性 benchmark，而非完整六分类主训练集。
...
### 6.6 实现细节
当前系统在 Agent 分支和直接基线分支中都使用 Qwen 兼容的 OpenAI-style 推理接口。为了避免隐藏的模型不一致问题，Agent 感知模块和报告模块会显式使用与 direct baseline 相同的模型名。评估导出中还会记录：
...
- `agent/`：负责编排、规划、路由、聚合和反思；
- `skills/`：负责感知、检索、元数据一致性、专科鉴别和报告；
- `memory/`：负责经验库、重排序、写回、checkpoint 与压缩；
- `scripts/`：负责训练、对比、消融、质量评估、显著性检验、图表导出和外部 DDI 评估。

### 6.7 统计分析
本文的配对比较采用 matched-case 统计方式。当前代码库已经补齐以下统计模块：

- McNemar 配对显著性检验；
- top-1 差值与 malignant recall 差值的 paired bootstrap 置信区间；
- 基于 ECE 的校准分析；
- 在存在元数据的情况下输出 subgroup 指标。

目前统计分析脚本已经写完，等待长训练后的最终结果导入即可得到正式论文数值。
...
## 11. 数据与代码可得性说明
当前代码库已经包含训练、对比、消融、质量分析、显著性检验、图表导出以及外部 DDI 评估所需的主要脚本。PAD-UFES-20 的 split manifest、learned checkpoint、compare 输出以及论文相关结果文件都统一保存在 `outputs/` 下。

数据可得性需要根据各数据集原始协议分别说明：

- PAD-UFES-20 作为主要内部实验集；
- DDI 作为外部 benchmark，需遵守其原始数据访问协议。

正式投稿前，应根据目标期刊要求，把这一节替换成最终公开仓库地址、版本说明和数据访问声明。

## 12. 作者贡献声明模板
当前草稿仍处于项目写作阶段，请在正式投稿前将以下模板替换为真实作者名单。
...
## 13. 经费支持声明模板
当前草稿中尚未写入经费支持信息。正式投稿前请补充项目资助、实验室支持、学校支持或自筹说明。

## 14. 利益冲突声明模板
作者声明不存在利益冲突。如正式投稿需要其他披露，请在此修改。
...
1. Agent vs Direct Qwen 的内部主对比柱状图；
2. 系统级 ablation 柱状图；
3. 内部 per-label F1 图；
4. 内部 calibration 曲线；
5. 内部 confusion matrix；
6. 外部 DDI binary 指标柱状图；
7. 外部 DDI subgroup 图（如 skin tone 切片）。
